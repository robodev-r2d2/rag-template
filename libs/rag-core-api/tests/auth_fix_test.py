...
def test_get_jwks_for_issuer_url_replacement(auth_middleware):
    with patch("rag_core_api.auth.requests.get") as mock_get:
        ...
        mock_get.assert_called_with("http://rag-keycloak-http:80/auth/realms/rag/protocol/openid-connect/certs", timeout=5)

def test_decode_with_jwks_leeway(auth_middleware):
    token = "mock_token"
    with patch("rag_core_api.auth.jwt.get_unverified_header") as mock_header, \
